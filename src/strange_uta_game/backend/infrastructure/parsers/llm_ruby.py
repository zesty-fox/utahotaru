...
_SYSTEM_PROMPT = (
    ...
    "(6) **拆字 vs 不拆の判定基準**："
    "ある漢字の読みが「その単漢字として独立に成立する読み（音読み・訓読み・"
    "それらの連濁/促音便/長母音化など音韻変化を経た派生形）」である場合は、"
    "字ごとに別ブロックに分けて書く（例：「物語」→ ``{物||もの}{語||がたり}``、"
    "「笑顔」→ ``{笑||え}{顔||かお}``、「毎日」→ ``{毎||まい}{日||にち}``、"
    "「大冒険」→ ``{大||だい}{冒||ぼう}{険||けん}``、「日本」→ ``{日||に}{本||ほん}``）。"
    "一方、「熟字訓・当て字・複合語特有の不規則読み（単漢字単独では成立しない読み）」"
    "の場合は、複数字をまとめて一ブロックにし、末尾に ``,`` ＋空読みで連結を示す"
    "（例：「今日（きょう）」→ ``{今日||きょう,}``、「昨日（きのう）」→ ``{昨日||きのう,}``、"
    "「大人」→ ``{大人||おとな,}``、「一日（ついたち）」→ ``{一日||ついたち,}``、"
    "「風邪」→ ``{風邪||かぜ,}``、「田舎」→ ``{田舎||いなか,}``、"
    "「七夕」→ ``{七夕||たなばた,}``、「紅葉」→ ``{紅葉||もみじ,}``）。"
    "判断基準：その読みを weblio や小学館デジタル大辞泉で当該単漢字の項目に"
    "辿れるなら拆字、辿れない（熟字訓として複合語の項目にのみ載る）なら不拆。"
    "迷ったら不拆（一ブロックにまとめる）方が安全。"
)

_OUTPUT_SCHEMA_HINT = (
    '出力フォーマット（JSON のみ）：\n'
    '{"lines":[{"i":0,'
    '"text":"{今日||きょう,}は{毎||まい}{日||にち}の{物||もの}{語||がたり}"}]}\n'
    "i は行番号、text はその行の注音済み inline annotated 形式。\n"
    "annotated 文法（プロジェクト共通の内部形式）：\n"
    "- ``{原文||読み区}`` —— 一つの形態素／まとまりを一ブロック。\n"
    "- 読み区は ``,`` で字ごとの読みを区切る（原文の文字数と一致）。\n"
    "- 末尾 ``,`` ＋空読み = その字は前字と連結（熟字訓の連音）。\n"
    "- 単字ブロック ``{字||読み}`` を並べると各字が独立した morpheme として扱われ、"
    "下流のユーザー辞書が個別に適用できる。\n"
    "- 多字ブロック ``{字字||読み,}`` は一つの morpheme としてまとめて保護され、"
    "下流のユーザー辞書がブロックの一部だけを上書きできない。\n"
    "- ブロック外は ``{}`` を付けず原文のまま（仮名/記号/英数字）。\n"
    "\n"
    "拆字（単字ブロックを並べる）— 独立読みが成立する場合：\n"
    "  「物語」  → ``{物||もの}{語||がたり}``（物=もの 訓、語=がたり は語る の連用名詞形）\n"
    "  「笑顔」  → ``{笑||え}{顔||かお}``（笑=え は笑む の語根、顔=かお 訓）\n"
    "  「毎日」  → ``{毎||まい}{日||にち}``（毎=まい 音、日=にち 音）\n"
    "  「大冒険」→ ``{大||だい}{冒||ぼう}{険||けん}``\n"
    "  「日本」  → ``{日||に}{本||ほん}``（連濁/促音便などの音変派生も独立読みの範疇）\n"
    "\n"
    "不拆（多字ブロック＋末尾連結）— 熟字訓・当て字・複合語特有の不規則読み：\n"
    "  「今日」  → ``{今日||きょう,}``（熟字訓；今=きょ や 日=う は単独読みではない）\n"
    "  「昨日」  → ``{昨日||きのう,}``（熟字訓）\n"
    "  「大人」  → ``{大人||おとな,}``（熟字訓）\n"
    "  「一日（ついたち）」→ ``{一日||ついたち,}``（熟字訓；いちにち の場合は 拆 してよい）\n"
    "  「風邪」  → ``{風邪||かぜ,}``（当て字）\n"
    "  「七夕」  → ``{七夕||たなばた,}``（熟字訓）\n"
    "\n"
