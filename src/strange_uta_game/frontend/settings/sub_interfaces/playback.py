...
class PlaybackSubInterface(SubSettingInterface):
    ...
    def _init_ui(self):
        ...
        self.card_volume = SpinSettingCard(FIF.VOLUME, tr("默认音量"), tr("音频加载后的初始音量"),
            min_val=0, max_val=100, suffix=" %", parent=g)
        self.card_speed = DoubleSpinSettingCard(FIF.SPEED_HIGH, tr("默认速度"), tr("音频加载后的初始播放速度"),
            min_val=0.2, max_val=2.0, step=0.05, decimals=2, suffix=" x", parent=g)
        self.card_speed_min = DoubleSpinSettingCard(FIF.SPEED_HIGH, tr("速度滑块最小值"), tr("播放栏速度滑块的下限"),
            min_val=0.2, max_val=2.0, step=0.05, decimals=2, suffix=" x", parent=g)
        self.card_speed_max = DoubleSpinSettingCard(FIF.SPEED_HIGH, tr("速度滑块最大值"), tr("播放栏速度滑块的上限"),
            min_val=0.2, max_val=2.0, step=0.05, decimals=2, suffix=" x", parent=g)
        self.card_fast_forward = SpinSettingCard(FIF.CHEVRON_RIGHT, tr("快进量"), tr("按下快进键跳过的时间"),
            min_val=1000, max_val=30000, step=1000, suffix=" ms", parent=g)
        self.card_rewind = SpinSettingCard(FIF.LEFT_ARROW, tr("快退量"), tr("按下快退键后退的时间"),
            min_val=1000, max_val=30000, step=1000, suffix=" ms", parent=g)
        self.card_auto_play = SwitchSettingCard(FIF.PLAY, tr("自动播放"), tr("加载音频文件后自动开始播放"), parent=g)
        self.card_hq_speed = SwitchSettingCard(
            FIF.SPEED_HIGH, tr("高质量倍速"),
            tr("开启后无爆音，但倍速切换可能有延迟，且需占用 .cache 进行离线预渲染；"
               "关闭后占用较小、切换即时，但可能会有爆音。默认开启。"),
            parent=g,
        )
        self.card_jump_before = SpinSettingCard(FIF.HISTORY, tr("删除节奏点跳转提前量"),
