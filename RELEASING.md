...
- **依赖真的变了**：改了 `requirements.txt` / `requirements-variants.txt` 里打进包的库，
...
改 `requirements.txt` / `requirements-variants.txt`、增删 `build.py` 的 `--exclude-module`
...
| 变体 | runner | 注音引擎 | 额外依赖 |
|------|--------|---------|---------|
| `main` | windows-latest | WinRT IME | `requirements.txt`（含 winrt-*） |
